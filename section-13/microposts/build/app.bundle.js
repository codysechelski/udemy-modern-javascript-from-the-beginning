...
function submitPost() {
  ...
  if (title === '' || body === '') {
    _ui.ui.showAlert('Post Title and Post Body are required', 'warning');
  } else {
    ...
    if (id === '') {
      ...
      _http.http.put('http://localhost:3000/posts/' + id, data).then(function (data) {
        _ui.ui.showAlert('Post Updated', 'success');
...
function deletePost(e) {
  if (e.target.classList.contains('delete') || e.target.parentElement.classList.contains('delete') || e.target.parentElement.parentElement.classList.contains('delete')) {

    //find the a tag
    var target = e.target;
    var link = e.target.nodeName.toLowerCase() === 'a' ? e.target : null;
    ...
    _http.http.delete('http://localhost:3000/posts/' + id).then(function (data) {
      _ui.ui.showAlert('Post Deleted', 'success');
      getPosts();
    }).catch(function (err) {
      return console.log(err);
    });
...
function enableEdit(e) {
  if (e.target.classList.contains('edit') || e.target.parentElement.classList.contains('edit') || e.target.parentElement.parentElement.classList.contains('edit')) {
    ...
    //once the a tag is fond, get he data-id
    var id = link.dataset.id;
    var body = link.previousElementSibling.textContent;
    var title = link.previousElementSibling.previousElementSibling.textContent;
